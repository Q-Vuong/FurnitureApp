    ...
    $("#productTypeForm").submit(function(e) {
      ...
      var formData = new FormData(this);
        ...
        $.ajax({
            ...
            processData: false,
            success: function(response) {
                // Load trang /productType/add-page?success=true vào phần content
...
document.getElementById('searchForm').addEventListener('submit', async function(event) {
  ...
  try {
    var url = `/productType/search?q=${searchInput}`; // Sử dụng template literals để tạo URL
    loadContent(url);
