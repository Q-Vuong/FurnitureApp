function loadContent(url) {
  ...
  $(document).ready(function() {
    ...
    $("#productForm").submit(function(e) {
        ...
        $.ajax({
            url: "/product/add", // Lấy url từ thuộc tính action của form
            type: "POST",
            data: formData,
            contentType: false,
            processData: false,
...
document.getElementById('searchForm').addEventListener('submit', async function(event) {
  ...
  try {
    var url = `/product/search?q=${searchInput}`; // Sử dụng template literals để tạo URL
    loadContent(url);
